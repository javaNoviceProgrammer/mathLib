...
public class SmoothInterpolation1D extends CubicSpline {
	...
	public SmoothInterpolation1D(double[] var1, double[] var2) {
		super(var1, var2);
	...
	public static void main(String[] args) {
		double[] x = MathUtils.linspace(0, 2*Math.PI, 100) ;
		double[] y1 = ArrayFunc.apply(t -> Math.sin(t*t), x) ;
		double[] y1deriv = ArrayFunc.apply(t -> Math.cos(t*t)*2*t, x) ;
		MatlabChart fig = new MatlabChart() ;
		fig.plot(x, y1deriv);
		// interpolate
		SmoothInterpolation1D interpolate = new SmoothInterpolation1D(x, y1) ;
